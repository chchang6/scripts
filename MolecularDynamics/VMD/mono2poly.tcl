proc parsematrix {orig_file}  {
  ...
  if {[llength matrixlist]==0} {
    error "There is no BIOMT REMARK information in this pdb file"
  }
...
proc mono2poly {args} {
  ...
  set output poly
  set chainlist {}
  puts -nonewline "commandlinelength "
  puts $cmdlinelength
  if {$cmdlinelength>=4} {
    ...
    while {$i<[llength $args]} {
      ...
      switch -exact -- $opt {
        ...
        -chain { 
          ...
          puts -nonewline "chain list: "
          puts $chainlist
          puts -nonewline "Length of chain list: "
          puts [llength $chainlist]
          puts -nonewline "Polynum: "
          puts $polynum
...
##
mol load pdb 2Z5R.pdb
set sel [atomselect top all]
##
set matrix [parsematrix 2Z5R.pdb]  
...
mono2poly -o 2Z5Rfull.pdb -chain {A} $sel $matrix  
