...
# Oxidized cluster--assumes '-v o' flag passed to script.
# Create list of lists with all possible orderings of 2 "3" and 2 "2" over
#   four centers. Six combinations (4!/(2!2!)).
# Reduced cluster--assumes '-v r' flag passed to script.
# Only 4 possible positions for ferric iron.
if options.valence == 'o':
   ...
   sys.exit('Valence must be set to "o" or "r"')

# For each possibility in newlist, modify the ironx_lists and ligand list charges.
listmap = { 0:iron1_list, 1:iron2_list, 2:iron3_list, 3:iron4_list, 4:lig_list, 5:iron1_dft, 6:iron2_dft, 7:iron3_dft, 8:iron4_dft }
for i in x:
   ...
   #   order as i and grouped.
   for j in range(0, len(lig_list.list)):
   ...
   # Set multiplicities in ironx_dft's
   for j in range(5,9):
   ...
   file_string = '63a' + str(i[0]) + '_64a' + str(i[1]) + \
   ...
   while j < len(pbsdata):
      if re.search('#PBS -N', pbsdata[j]):
         if options.valence == 'r':
            k = '#PBS -N Bred_' + str(i[0]) + str(i[1]) + str(i[2]) + str(i[3]) + '\n'
         else:
            k = '#PBS -N Box_' + str(i[0]) + str(i[1]) + str(i[2]) + str(i[3]) + '\n'
